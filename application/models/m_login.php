...
class M_login extends CI_Model {
	...
	*/    
    function ValUser($user,$password)
    {
        ...
        $this->db->select($this->table.'.*,grupo.nombre as grupo,grupo.url_index as url_grupo,departamento.nombre as departamento');
        $this->db->from($this->table);
        $this->db->join('grupo', 'users.grupo_id = grupo.id', 'left');
        $this->db->join('departamento', 'users.departamento_id = departamento.id', 'left');
